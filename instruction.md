...
months of bridge notes are in `/app/incident/export_dossier.md`. most of it is noise and some early triage threads contradict later findings — treat the dossier as the source of truth for *behavior*, not every acquirer-window blurb. output shape and audit schemas are in `/app/docs/report_spec.json`.
